...
int loginAccount(std::string email, std::string password, std::string ipAddr); //returns user_id ( > 0 ) = ok, 0 = login ok but account is banned (accounts.active = 0), -1 = wrong email/password
