...
int createAccount(mysqlpp::Connection &con, std::string email, std::string userName, std::string password, std::string ipAddr); //returns user_id ( > 0 ) = ok, -1 = email/username exists, -2 = unspecified error
